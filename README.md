...
최근 1RM, 주간 볼륨, 총 볼륨, 최근 4주 요약, 목표 진행률을 확인합니다. 첫 방문자는 `1RM 계산 -> 세션 기록 -> 목표 설정` 순서로 시작할 수 있습니다.
...
기간별 세션 수, 총 볼륨, 평균 1RM, PR 후보, RPE 기반 강도 해석, 종목별 흐름을 확인합니다. 기록이 없을 때는 계산기로 이동하는 CTA를 제공합니다.
...
| 백업/복원 | JSON 내보내기/가져오기로 로컬 데이터를 보존합니다. |
...
| 화면 | 용도 |
|---|---|
| 대시보드 | 전체 운동 상태, 최근 기록, 4주 볼륨 요약, 목표 진행률을 봅니다. |
| 1RM 계산기 | 1RM 계산과 세션 기록 생성/수정/삭제를 수행합니다. |
| 분석 | 기간별 통계, PR 후보, RPE 해석, 차트를 확인합니다. |
...
src/
  app/                  앱 레이아웃
  components/common/    네비게이션, PWA 상태, 오류 경계, 진단 수집
